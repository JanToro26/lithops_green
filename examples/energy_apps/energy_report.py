...
Usage from an app:
    ...
    profile_map('my_app', my_function, make_iterdata, config_space)
"""
import os
import csv
import time
...
def print_summary(app, cfg, summ, wall):
    print(f"[{app}] workers={summ['workers']:<3} mem={cfg.get('memory')} "
          f"wall={wall:6.2f}s dur={summ['max_duration_s']:6.2f}s "
          f"E_psutil={summ['psutil_pkg_j']:8.2f}J E_rapl={summ['rapl_pkg_j']:8.2f}J "
          f"cpu={summ['avg_cpu_pct']:4.1f}% [{summ['method']}]")


def append_csv(path, app, cfg, summ, wall):
    os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
    new = not os.path.exists(path)
    with open(path, 'a', newline='') as f:
        w = csv.writer(f)
        if new:
            w.writerow(['app', 'workers', 'memory', 'wall_s', 'max_duration_s',
                        'psutil_pkg_j', 'rapl_pkg_j', 'avg_cpu_pct', 'method', 'tstamp'])
        w.writerow([app, summ['workers'], cfg.get('memory'), round(wall, 3),
...
def profile_map(app, func, make_iterdata, config_space,
                ...
                csv_path=None, reduce_fn=None):
    """
    Run 'func' for each configuration in 'config_space' (a list of dicts with at
    least {'workers': N} and optionally {'memory': MB}), collecting energy.

    make_iterdata(workers) -> list of arguments (one per worker).
    reduce_fn(results)     -> optional, to combine results (e.g. estimate pi).
    """
    import lithops
    csv_path = csv_path or DEFAULT_CSV
    print(f"== Profiling '{app}' on backend '{backend}' ==")
    for cfg in config_space:
        ...
        summ = summarize(futs)
        print_summary(app, cfg, summ, wall)
        append_csv(csv_path, app, cfg, summ, wall)
...
def profile_pipeline(app, run_fn, config_space,
                     backend='localhost', storage='localhost', csv_path=None):
    """
    Like profile_map but for MULTI-STAGE workflows.

    run_fn(fexec, workers) runs all stages (several fexec.map calls with
    dependencies) and returns the list of ALL futures produced. Energy is
    aggregated over every invocation of every stage.
    """
    import lithops
    csv_path = csv_path or DEFAULT_CSV
    print(f"== Profiling '{app}' (pipeline) on backend '{backend}' ==")
    for cfg in config_space:
        workers = cfg['workers']
        memory = cfg.get('memory')
        kwargs = dict(backend=backend, storage=storage)
